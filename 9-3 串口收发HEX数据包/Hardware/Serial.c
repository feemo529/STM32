...
uint8_t Serial_RxPacket[4]; // 存储接受的载荷数据
uint8_t Serial_RxFlag;

// 1、开启时钟，把需要用的USART和GPIO的时钟打开
// 2、GPIO初始化，把TX配置呈复用输出，RX配置成输入
// 3、配置USART
...
void Serial_Init(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannel = USART1_IRQn; // 开启中断通道
...
void Serial_SendPacket(void)
{
    ...
    Serial_SendArray(Serial_TxPacket, 4); // 载荷数据
