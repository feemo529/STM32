...
void Timer_Init(void)
{
    ...
    TIM_ETRClockMode2Config(TIM2, TIM_ExtTRGPSC_OFF, TIM_ExtTRGPolarity_NonInverted, 0x00); // 选择时基单元的时钟,通过ETR引脚的外部时钟模式2配置，
                                                                                            // 第二个参数是外部触发预分频器，第三个参数是外部触发的的极性,第四个参数是外部触发滤波器
